...
/* Función para medir la longitud del pulso entrante. Mide el tiempo que transcurrido entre el envío
  ...
  pulseTime=pulseIn(Echo, HIGH, 30000);
  ...
  if (pulseTime == 0) {
    ...
    delay(1);
    pinMode(Echo,INPUT);
    pulseTime = 9999; // means out of range
  }
...
long microsecondsToCentimeters(long microseconds) {
  ...
  //return (microseconds / 29.4117647 /2); // adjust on linear regression
